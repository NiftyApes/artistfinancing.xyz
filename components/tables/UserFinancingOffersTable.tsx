...
const UserListingsTableRow = ({ offer, status, signature, token }: UserOffersRowProps) => {
...
const UserFinancingOffersTableMobileRow = ({ offer, token }: UserOffersMobileRowProps) => {
  
  const { listPrice, expirationRelative, image, tokenName, collectionName } = processOffer(offer, token)
    ...
    <div className='border-b-[1px] border-solid border-b-neutral-300	py-[16px]'>
      <div className='flex items-center justify-between'>
        <div className="flex items-center gap-2">
          ...
          <div>
            <div className="reservoir-h6 max-w-[200px] overflow-hidden text-ellipsis whitespace-nowrap font-headings text-sm dark:text-white text-start">
              {tokenName ? tokenName : collectionName}
            </div>
            {tokenName && (
