...
export default function MakePaymentModal({ data, loan, offer }: {
  ...
  const payment: BigNumber = BigNumber.from(loan.minimumPrincipalPerPeriod)
    .add(BigNumber.from(loan.remainingPrincipal).mul(BigNumber.from(loan.periodInterestRateBps))
        ...
        <ModalContent>
          ...
          <ModalBody p='0'>
            <Flex>
              ...
              <Box p='6' w='full'>
                {paymentTransaction && isSuccess && `Thanks for your payment. Transaction processed successfully. `}
                {paymentTransaction && isLoading && `Processing transaction... `}
                {paymentTransaction && isError && `Unable to process transaction... `}
                {paymentTransaction && (
                  <Box>
                    <Link href={`${etherscanUri}/tx/${paymentTransaction.hash}`} isExternal>View Transaction</Link>
                  </Box>
                )
                }
                {!paymentTransaction && (
                    ...
                    <div className='flex w-full flex-row items-center justify-between rounded-md bg-gray-700 p-4'>
                      <div className='flex flex-col items-center gap-2'>
                        <div className='flex flex-row items-center gap-2 text-xl font-semibold'>
                          <p>{Number(formatEther(loan.remainingPrincipal)).toFixed(4)}</p>
                          <AiOutlineArrowRight />
                          <p
                            className='text-green-400'>{Number(formatEther(BigNumber.from(loan.remainingPrincipal).sub(BigNumber.from(loan.minimumPrincipalPerPeriod)))).toFixed(4)}</p>
                        </div>
                      ...
                      <div className='flex flex-col items-center gap-2'>
                        <div className='flex flex-row items-center gap-2 text-xl font-semibold'>
                          <p><FormatNativeCrypto maximumFractionDigits={4} amount={payment} /></p>
                        </div>
