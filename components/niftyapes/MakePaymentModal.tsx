...
export default function MakePaymentModal({
  ...
  const { isLoading: isLoadingTxn, isSuccess: isSuccessTxn, isError: isErrorTxn } = useWaitForTransaction({
          ...
          <ModalBody p='0'>
            <Flex>
              ...
              <Box p='6' w='full'>
                ...
                {paymentTxn &&
                  <Box>
                    {isSuccessTxn && `Thanks for your payment. Transaction processed successfully. `}
                    {isLoadingTxn && `Processing transaction... `}
                    {isErrorTxn && `Unable to process transaction... `}

                    <Link href={`${etherscanUri}/tx/${paymentTxn?.hash}`} isExternal>View Transaction</Link>
                  </Box>
                }
